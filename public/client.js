...
function getProfile() {
    axios({
        ...
        document.getElementById("img").src = response.data.profile.profilePic}
            ...
            document.getElementById("profilePic").src = "https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png"
        }
...
function getTweets() {
    axios({
        ...
        console.log('kch bhi' + response.data.data)
        let tweets = response.data.data;
        let html = ""
        for (let i = 0; i < tweets.length; i++) {
            html += `
            <div class="posts">
            <img src="${tweets[i].profilePic}" style="width:50px;height:50px;border-radius:100%;" />
            <span style="font-size:22px;">${tweets[i].name}</span>
            <p>${tweets[i].tweets}</p>
            </div>
            `
        }
...
socket.on('NEW_POST', (newPost) => {
    ...
    <div class="posts">
    <img src="${tweets.profilePic}" style="width:50px;height:50px;border-radius:100%;" />
    <span style="font-size:22px;">${tweets.name}</span>
    <p>${tweets.tweets}</p>
    </div>
    `
})
